  ...
  Future<void> _sendMessage(String text) async {
      ...
      try {
        ...
        if (response.statusCode == 200 || response.statusCode == 202) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                _responseMessage(response, 'error_sending_message'),
              ),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                AppLocalizations.of(context).translate('error_sending_message'),
              ),
              backgroundColor: Colors.red,
            ),
          );
    ...
    try {
      ...
      if (_selectedChannel == 'private' && !isCommand) {
        ...
        if (replyTarget == null) {
          _selectedPrivatePeer = _privateConversationLabel(
            _monitorPlayerName,
            parsed['target']!,
          );
        }
      ...
      if ((isCommand ||
              ...
              _selectedChannel == 'commands') &&
          mounted) {
        setState(() => _selectedChannel = 'global');
      }
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              AppLocalizations.of(context).translate(
                isCommand ? 'god_command_queued' : 'chat_command_queued',
              ),
            ),
            ...
            child: Column(
              ...
              children: [
                Icon(
                  ...
                  size: 48,
                  color: _channelColor.withAlpha(179),
                ),
                SizedBox(height: _getAdaptivePadding(16)),
                Text(
