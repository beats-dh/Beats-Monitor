...
class PlayerInfoScreen extends StatefulWidget {
  ...
  const PlayerInfoScreen({Key? key, required this.playerName}) : super(key: key);
  ...
  Future<Map<String, dynamic>> fetchPlayerInfo(String name) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (root['dados'] != null) {
        ...
        throw Exception(AppLocalizations.of(context).translate('player_not_found'));
      }
    } else if (response.statusCode == 404) {
      throw Exception(AppLocalizations.of(context).translate('player_not_found'));
    } else {
      throw Exception(AppLocalizations.of(context).translate('player_info_error'));
    }
  }
              ...
              builder: (context, snapshot) {
                ...
                Widget buildVocationIcon(String? vocation) {
                  switch ((vocation ?? '').toLowerCase()) {
                    case 'knight':
                      return const Icon(Icons.shield, color: Colors.blueAccent, size: 28);
                    case 'paladin':
                      return const Icon(Icons.architecture, color: Colors.orangeAccent, size: 28);
                    case 'sorcerer':
                ...
                return Padding(
                  ...
                  child: ListView(
                    children: [
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Row(
                                          children: [
                                            Text('${l10n.translate('level')}: ', style: const TextStyle(fontWeight: FontWeight.bold)),
                                            Text('${data['level'] ?? '-'}'),
                                            const SizedBox(width: 16),
                                            Text('${l10n.translate('vocation')}: ', style: const TextStyle(fontWeight: FontWeight.bold)),
                                            Text(data['vocation'] ?? '-')
                                          ],
                                        ),
                                      ],
                              ...
                              buildBar(l10n.translate('hp'), data['health'] ?? 0, data['max_health'] ?? 1, Colors.red),
                              const SizedBox(height: 10),
                              buildBar(l10n.translate('mp'), data['mana'] ?? 0, data['max_mana'] ?? 1, Colors.blue),
                              const SizedBox(height: 16),
                              Row(
                                children: [
                                  Icon(
                                    data['online'] == true ? Icons.circle : Icons.circle_outlined,
                                    color: data['online'] == true ? Colors.green : Colors.red,
                                    size: 18,
                                  ),
                                  const SizedBox(width: 6),
                                  Text(l10n.translate('online'), style: const TextStyle(fontWeight: FontWeight.bold)),
                                  const SizedBox(width: 18),
                                  Icon(
                                    data['premium'] == true ? Icons.star : Icons.star_border,
                                    color: Colors.amber,
                                    size: 18,
                                  ),
                                  const SizedBox(width: 6),
                                  Text(l10n.translate('premium'), style: const TextStyle(fontWeight: FontWeight.bold)),
                                ],
                              ),
                            ],
                          ),
                      ...
                      Text(l10n.translate('skills'), style: Theme.of(context).textTheme.titleMedium),
                      const SizedBox(height: 10),
                      if (skills == null)
                        Text('-')
                      else
                              ...
                              children: [
                                for (final skill in skillOrder)
                                  Container(
                                    ...
                                    child: Row(
                                      children: [
                                        Icon(skillIcons[skill], size: 20, color: Theme.of(context).colorScheme.primary),
                                        const SizedBox(width: 8),
                                        Text(l10n.translate(skill), style: const TextStyle(fontWeight: FontWeight.bold)),
                                        const SizedBox(width: 8),
                                        Text('${skills[skill] ?? '-'}'),
                                      ],
                                    ),
                      ...
                      // Histórico de ban
                      if (banSnapshot.hasData && (banSnapshot.data?.isNotEmpty ?? false)) ...[
                        const SizedBox(height: 24),
                        Text(l10n.translate('ban_history_title'), style: Theme.of(context).textTheme.titleMedium),
                        const SizedBox(height: 8),
                        ...banSnapshot.data!.map((ban) {
                          final dt = DateTime.fromMillisecondsSinceEpoch((ban['banned_at'] ?? 0) * 1000);
                          final dtEnd = DateTime.fromMillisecondsSinceEpoch((ban['expired_at'] ?? 0) * 1000);
                          return Card(
                            color: Colors.red.withOpacity(0.07),
                            margin: const EdgeInsets.symmetric(vertical: 6),
                            child: ListTile(
                              leading: const Icon(Icons.gavel, color: Colors.red),
                              title: Text(ban['reason'] ?? '-', style: const TextStyle(fontWeight: FontWeight.bold)),
                              subtitle: Text('${l10n.translate('ban_start')}: ${dt.day.toString().padLeft(2, '0')}/${dt.month.toString().padLeft(2, '0')}/${dt.year} ${dt.hour.toString().padLeft(2, '0')}:${dt.minute.toString().padLeft(2, '0')}\n${l10n.translate('ban_end')}: ${dtEnd.day.toString().padLeft(2, '0')}/${dtEnd.month.toString().padLeft(2, '0')}/${dtEnd.year} ${dtEnd.hour.toString().padLeft(2, '0')}:${dtEnd.minute.toString().padLeft(2, '0')}'),
                            ),
                          );
                        }).toList(),
                      ],
